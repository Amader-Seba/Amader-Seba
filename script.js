// ==================== কনফিগ ====================
const API_URL = 'https://script.google.com/macros/s/AKfycbzrATG_bFIaFAYgcwJ91rsEEHgXYJHv7ODRqj1r36Va0JF9uLRpqDErUJUCGIhO6y6Z/exec'; // আপনার URL বসান
...
async function apiCall(action, data = {}) {
  ...
  try {
    ...
    console.error(e);
    hideSpinner();
    toast('নেটওয়ার্ক সমস্যা! URL চেক করুন।');
...
function updateNav() {
  ...
  const home = $('homeBtn'), myOrders = $('myOrdersBtn'), adminBtn = $('adminPanelBtn');
  const login = $('loginBtn'), register = $('registerBtn'), logoutBtn = $('logoutBtn');
  if (currentUser) {
...
async function renderHome() {
  ...
  const parents = [
    { id: 'nid_card', name: 'এনআইডি কার্ড', icon: 'fa-id-card', children: parentGroups.nid_card },
    { id: 'sim_biometric', name: 'সিম বায়োমেট্রিক', icon: 'fa-sim-card', children: parentGroups.sim_biometric },
    { id: 'sim_location', name: 'সিম লোকেশন', icon: 'fa-map-marker-alt', direct: true, serviceId: 'sim_location' },
    { id: 'lost_id', name: 'হারানো আইডি', icon: 'fa-search', direct: true, serviceId: 'lost_id' },
    { id: 'nid_to_sim', name: 'এনআইডি টু সিম', icon: 'fa-link', direct: true, serviceId: 'nid_to_sim' },
    { id: 'lost_etin', name: 'হারানো ই-টিন', icon: 'fa-file-invoice', direct: true, serviceId: 'lost_etin' },
    { id: 'imei_active', name: 'IMEI টু নম্বর', icon: 'fa-mobile-alt', direct: true, serviceId: 'imei_active' },
    { id: 'lost_nid_form', name: 'হারানো এনআইডি ফরম', icon: 'fa-file-alt', children: parentGroups.lost_nid_form },
    { id: 'birth_reg', name: 'জন্ম নিবন্ধন', icon: 'fa-baby', children: parentGroups.birth_reg }
  ];
  ...
  parents.forEach(p => {
    ...
    card.onclick = () => { if (p.direct) openServicePage(p.serviceId); else renderSubServices(p.id, p.name, p.children); };
...
function renderSubServices(parentId, title, childIds) {
  ...
  app.innerHTML = `<button class="back-btn" onclick="renderHome()">← ফিরে যান</button>
    ...
    <div class="sub-service-grid" id="subGrid"></div>
  ...
  // টেবিল লোড করুন (কার্ড সিলেক্ট না করলেও)
  loadServiceOrdersTableForAll(childIds);
}

async function loadServiceOrdersTableForAll(childIds) {
  const container = document.getElementById('ordersTableContainer');
  if (!container) return;
  const res = await apiCall('getOrders', { userId: currentUser.userId });
  const allOrders = res.orders || [];
  // সব সার্ভিসের অর্ডার দেখান যেগুলো এই গ্রুপের আওতায়
  const orders = allOrders.filter(o => childIds.includes(o.serviceId));
  
  if (orders.length === 0) { 
    container.innerHTML = '<p style="text-align:center;padding:20px;">কোনো অর্ডার নেই</p>'; 
    return; 
  }
  
  let html = `<table><thead><tr><th>SL</th><th>সার্ভিস</th><th>স্ট্যাটাস</th><th>মেসেজ</th></tr></thead><tbody>`;
  orders.forEach((o, idx) => {
    ...
    if (o.status === 'delivered') {
      if (o.deliveryLink) {
        messageHtml = `<a href="${o.deliveryLink}" target="_blank" class="download-btn">ডাউনলোড</a>`;
      } else if (o.deliveryText) {
        messageHtml = `<button class="msg-btn" data-msg="${o.deliveryText.replace(/"/g, '&quot;')}">মেসেজ</button>`;
      } else {
        messageHtml = 'সম্পন্ন';
      }
    ...
    html += `<tr><td>${idx+1}</td><td>${serviceForms[o.serviceId]?.name || o.serviceId}<tr>
      <td><span class="status-badge ${statusClass}">${statusText}</span></td>
      <td>${messageHtml} ${detailsBtn}</td></tr>`;
  });
  html += `</tbody></table>`;
  ...
  document.querySelectorAll('.detail-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      try { 
        ...
        raw = Object.entries(obj).map(([k,v])=>`${k}: ${v}`).join('\n'); 
      } catch(e) {}
      showModal(raw, 10000);
    });
  });
...
function showOrderForm(serviceId) {
  ...
  container.innerHTML = `
    <h3 style="margin-bottom:12px;">${s.name} অর্ডার করুন</h3>
    <div style="background:#e0f2e9;padding:10px;border-radius:12px;margin-bottom:12px;">💰 চার্জ: ${s.rate} টাকা</div>
    <div style="display:flex; gap:8px; align-items:center; margin-bottom:15px;">📞 নগদ: <b>${NAGAD_NUMBER}</b> <button onclick="copyText('${NAGAD_NUMBER}')" style="background:#2c6b52;color:white;border:none;padding:4px 12px;border-radius:20px;">কপি</button></div>
    <form id="orderForm">
  ...
  document.getElementById('orderForm').addEventListener('submit', async (e) => {
    ...
    if (res.success) { 
      ...
      // রিফ্রেশ করতে parentGroups এর চাইল্ড আইডিস পান
      let allChildIds = [];
      Object.values(parentGroups).forEach(ids => allChildIds = [...allChildIds, ...ids]);
      allChildIds = [...allChildIds, 'sim_location', 'lost_id', 'nid_to_sim', 'lost_etin', 'imei_active'];
      loadServiceOrdersTableForAll(allChildIds);
...
function openServicePage(serviceId) {
  ...
  loadOrdersTableForService(serviceId);
  ...
  document.getElementById('orderForm').addEventListener('submit', async (e) => {
    ...
    for (let f of s.fields) { if (f.required && !formData[f.id]) { toast(`${f.name} পূরণ করুন`); btn.disabled = false; return; } }
    const res = await apiCall('placeOrder', { userId: currentUser.userId, serviceId, formData: JSON.stringify(formData), transactionId: txnId });
    if (res.success) { toast('অর্ডার সফল!'); document.getElementById('txnId').value = ''; loadOrdersTableForService(serviceId); }
    else toast(res.message || 'অর্ডার ব্যর্থ');
    btn.disabled = false;
  });
}

async function loadOrdersTableForService(serviceId) {
  const container = document.getElementById('ordersTableContainer');
  if (!container) return;
  const res = await apiCall('getOrders', { userId: currentUser.userId });
...
function showModal(content, autoCloseMs = 10000) {
  const modal = $('modal');
  const modalBody = $('modalBody');
  modalBody.innerHTML = `<pre style="white-space:pre-wrap;margin:10px 0;">${content}</pre>`;
  modal.style.display = 'flex';
  document.querySelector('.modal-close').onclick = () => modal.style.display = 'none';
  if (autoCloseMs) setTimeout(() => modal.style.display = 'none', autoCloseMs);
}

function showModalWithCopy(content, autoCloseMs = 10000) {
  const modal = $('modal');
  const modalBody = $('modalBody');
  modalBody.innerHTML = `<pre style="white-space:pre-wrap;margin:10px 0;">${content}</pre>
    <button class="copy-modal-btn" id="copyMsgBtn">📋 কপি করুন</button>`;
  modal.style.display = 'flex';
...
async function renderMyOrders() {
  ...
  orders.forEach((o, i) => {
    ...
    html += `<tr><td>${i+1}</td><td>${serviceForms[o.serviceId]?.name || o.serviceId}</td>
      <td><span class="status-badge ${statusClass}">${o.status === 'pending' ? 'পেন্ডিং' : (o.status === 'delivered' ? 'সফল' : 'বাতিল')}</span></td>
      <td>${messageHtml} ${detailsBtn}</td></tr>`;
  });
  ...
  document.querySelectorAll('.msg-btn').forEach(btn => {
    btn.addEventListener('click', () => showModalWithCopy(btn.getAttribute('data-msg'), 10000));
  });
}
  ...
  if (tab === 'dashboard') {
    ...
    (res.orders || []).forEach(o => {
      html += `<tr><td>${o.orderId}</td><td>${o.userId}</td><td>${serviceForms[o.serviceId]?.name || o.serviceId}</td>
        ...
        <td><button onclick="deliverOrder(${o.orderId})" style="background:#27ae60;color:white;border:none;padding:4px 10px;border-radius:16px;margin:2px;">ডেলিভার</button>
        <button onclick="cancelOrder(${o.orderId})" style="background:#e74c3c;color:white;border:none;padding:4px 10px;border-radius:16px;">বাতিল</button></td></tr>`;
    });
    html += `</tbody></table>`;
    content.innerHTML = html;
  } else if (tab === 'users') {
    const res = await apiCall('getUsers');
    let html = `<table><thead><tr><th>ID</th><th>নাম</th><th>ফোন</th><th>রোল</th><th>স্ট্যাটাস</th><th>অ্যাকশন</th></tr></thead><tbody>`;
    (res.users || []).forEach(u => {
      html += `<tr><td>${u.userId}</td><td>${u.name}</td><td>${u.phone}</td>
        <td>${u.role || 'user'}</td><td>${u.isBlocked ? '🚫 ব্লক' : '✅ সক্রিয়'}</td>
        <td><button onclick="blockUser('${u.userId}',${!u.isBlocked})" style="background:#2980b9;color:white;border:none;padding:4px 10px;border-radius:16px;">${u.isBlocked ? 'আনব্লক' : 'ব্লক'}</button></td></tr>`;
    });
    html += `</tbody></table>`;
    content.innerHTML = html;
  } else if (tab === 'notice') {
    const res = await apiCall('getNotice');
    content.innerHTML = `<textarea id="noticeInput" style="width:100%;height:80px;padding:10px;border-radius:12px;">${res.notice || ''}</textarea>
      <button class="btn-primary" onclick="updateNotice()" style="margin-top:10px;">নোটিশ আপডেট করুন</button>`;
  }
}
...
async function blockUser(userId, block) {
  await apiCall('blockUser', { userId, block });
  adminTab('users');
}

async function updateNotice() {
  const txt = document.getElementById('noticeInput')?.value || '';
  await apiCall('updateNotice', { text: txt });
  toast('নোটিশ আপডেট হয়েছে');
}

function showLoginForm() {
  $('app').innerHTML = `<h2 class="page-title">লগইন</h2>
    <div class="form-group"><label>মোবাইল</label><input id="loginPhone"></div>
    <div class="form-group"><label>পাসওয়ার্ড</label><input type="password" id="loginPass"></div>
    <button class="btn-primary" onclick="doLogin()">লগইন</button>
    <p style="margin-top:10px;text-align:center">নতুন? <a href="#" onclick="showRegisterForm()">রেজিস্টার</a></p>`;
}

function showRegisterForm() {
  $('app').innerHTML = `<h2 class="page-title">রেজিস্টার</h2>
    <div class="form-group"><label>নাম</label><input id="regName"></div>
    <div class="form-group"><label>মোবাইল</label><input id="regPhone"></div>
    <div class="form-group"><label>পাসওয়ার্ড</label><input type="password" id="regPass"></div>
    <button class="btn-primary" onclick="doRegister()">রেজিস্টার</button>
    <p style="margin-top:10px;text-align:center">অ্যাকাউন্ট আছে? <a href="#" onclick="showLoginForm()">লগইন</a></p>`;
}
...
function setupEvents() {
  document.querySelectorAll('.sub-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (action === 'home') currentUser ? (currentUser.role === 'admin' ? renderAdminPanel() : renderHome()) : renderLandingPage();
  ...
  $('homeBtn').onclick = () => currentUser ? (currentUser.role === 'admin' ? renderAdminPanel() : renderHome()) : renderLandingPage();
  $('myOrdersBtn').onclick = renderMyOrders;
  $('adminPanelBtn').onclick = renderAdminPanel;
