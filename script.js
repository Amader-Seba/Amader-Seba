// ==================== কনফিগ ====================
const API_URL = 'https://script.google.com/macros/s/AKfycbxKKRzUWdr6CnjV04OhAUtRVLfN9sSdIIXdz-uVhqv48ROVBAATV-IAnA3o238bdQUN/exec'; // আপনার URL বসান
...
async function showOrderFormInline(serviceId) {
  ...
  container.innerHTML = `
    ...
    <div class="order-table-wrapper"><h3 style="margin-top:20px;">📋 আপনার অর্ডারসমূহ</h3><div id="ordersTableContainer"></div></div>
  `;
  await loadServiceOrdersTable(serviceId);
  document.getElementById('orderForm').addEventListener('submit', async (e) => {
    ...
    if (res.success) { toast('অর্ডার সফল!'); document.getElementById('txnId').value = ''; await loadServiceOrdersTable(serviceId); }
    else toast(res.message || 'অর্ডার ব্যর্থ');
    btn.disabled = false;
  });
}

async function loadServiceOrdersTable(serviceId) {
  ...
  orders.forEach((o, idx) => {
    ...
    let detailsBtn = `<button class="detail-btn" data-order='${JSON.stringify(o.formData)}'>তথ্য</button>`;
    html += `<tr><td>${idx+1}</td><td>${serviceForms[o.serviceId]?.name || o.serviceId}</td>
...
function openOrderForm(serviceId) {
  ...
  loadServiceOrdersTable(serviceId);
  document.getElementById('orderForm').addEventListener('submit', async (e) => {
    e.preventDefault();
    const btn = document.getElementById('submitOrderBtn');
    btn.disabled = true;
    ...
    for (let f of s.fields) { if (f.required && !formData[f.id]) { toast(`${f.name} পূরণ করুন`); btn.disabled = false; return; } }
    const res = await apiCall('placeOrder', { userId: currentUser.userId, serviceId, formData: JSON.stringify(formData), transactionId: txnId });
    if (res.success) { toast('অর্ডার সফল!'); document.getElementById('txnId').value = ''; loadServiceOrdersTable(serviceId); }
    else toast(res.message || 'অর্ডার ব্যর্থ');
    btn.disabled = false;
  });
}
...
function showModal(content, autoCloseMs = 10000, showCopy = false) {
  ...
  modalBody.innerHTML = `<pre style="white-space:pre-wrap;margin:10px 0;">${content}</pre>${showCopy ? '<button class="copy-modal-btn" style="background:#2c6b52;color:white;border:none;padding:8px 16px;border-radius:20px;cursor:pointer;">কপি করুন</button>' : ''}`;
...
function showLoginForm() {
  $('app').innerHTML = `<h2 class="page-title">লগইন</h2>
    ...
    <p style="margin-top:8px;font-size:0.8rem;color:#666;">অ্যাডমিন: Role='admin' থাকলে লগইন করলেই অ্যাডমিন প্যানেল দেখাবে</p>`;
}
