...
function simulateLocalApi(action, p) {
  ...
  if (action === 'submitRecharge') {
    ...
    const newRecharge = { rechargeId: newId, userId: p.userId, phone: p.phone, name: p.name || 'Unknown', amount, transactionId: (p.transactionId || '').trim(), status: 'pending', createdAt: new Date().toISOString() };
  ...
  if (action === 'updateRechargeStatus') {
    ...
    if (idx !== -1) {
      ...
      saveLocalRecharges(recharges);
      // 🔥 এডমিন এপ্রুভাল: ব্যালেন্স অ্যাড করো
      if (oldStatus !== 'approved' && newStatus === 'approved') {
        ...
        if (userIdx !== -1) {
          ...
          saveLocalUsers(users);
          console.log(`Balance added: ${recharges[idx].amount} to user ${recharges[idx].userId}, new balance: ${users[userIdx].balance}`);
        }
      }
      return { success: true };
    }
...
// Complete service forms with all fields
const serviceForms = {
...
const allServices = [
  { id: 'sim_location', name: 'সিম নাম্বার লোকেশন', icon: 'MapPin', group: null },
  { id: 'lost_id', name: 'হারানো আইডি কার্ড', icon: 'Search', group: null },
  { id: 'nid_to_sim', name: 'এনআইডি টু সকল সিম', icon: 'Share2', group: null },
  { id: 'lost_etin', name: 'হারানো ই-টিন', icon: 'FileText', group: null },
  { id: 'imei_active', name: 'IMEI To Active', icon: 'Smartphone', group: null }
];

function getIcon(name) {
  const icons = { HandHeart: '🤲', HomeIcon: '🏠', Coins: '💰', Settings: '⚙️', Users: '👥', RefreshCw: '🔄', Eye: '👁️', Download: '⬇️', Copy: '📋', Check: '✓', X: '✕', Menu: '☰', LogIn: '🔑', UserPlus: '👤+', LogOut: '🚪', IdCard: '🪪', Smartphone: '📱', MapPin: '📍', FileText: '📄', Baby: '👶', Megaphone: '📢', Info: 'ℹ️', Search: '🔍', Share2: '📤' };
  return icons[name] || '📦';
}
...
function App() {
  ...
  React.useEffect(() => {
    if (currentUser && currentUser.role !== 'admin') {
      const interval = setInterval(() => { refreshUserBalance(); if (currentScreen === 'recharge') loadUserRecharges(); }, 15000);
  ...
  return React.createElement('div', { className: 'min-h-screen bg-slate-50' },
    ...
    React.createElement('nav', { className: 'sticky top-0 z-50 bg-[#0a2e1f] text-white shadow-md' },
        ...
        React.createElement('div', { className: 'flex justify-between h-16 items-center' },
          React.createElement('div', { className: 'flex items-center gap-2 cursor-pointer', onClick: () => navigateTo(currentUser ? (currentUser.role === 'admin' ? 'admin' : 'home') : 'landing') },
            ...
            React.createElement('div', null, 
              React.createElement('span', { className: 'font-extrabold text-xl block' }, 'Amader Seba'),
              React.createElement('span', { className: 'text-[10px] text-emerald-400 block' }, 'সেবাই মূল লক্ষ্য')
            )
          ),
      ...
      hamburgerOpen && React.createElement('div', { className: 'md:hidden bg-[#0d3b28] p-4 space-y-2' },
        currentUser ? (
          React.createElement(React.Fragment, null,
            React.createElement('div', { className: 'bg-emerald-950 p-3 rounded-xl' }, 
              React.createElement('p', { className: 'text-emerald-400 text-xs' }, 'লগইন:'),
              React.createElement('p', { className: 'text-white font-bold' }, currentUser.name)
            ),
    ...
    React.createElement('main', { className: 'max-w-7xl mx-auto px-4 py-8' },
            ...
            React.createElement('table', { className: 'w-full text-sm' },
              React.createElement('thead', null,
                React.createElement('tr', { className: 'bg-slate-50 border-b' },
                  ...
                  React.createElement('th', { className: 'p-2 text-left' }, 'তারিখ'),
                  React.createElement('th', { className: 'p-2 text-left' }, 'স্ট্যাটাস')
                )
              ),
              React.createElement('tbody', null, 
                userRecharges.length === 0 ? 
                  React.createElement('tr', null, React.createElement('td', { colSpan: 5, className: 'p-4 text-center text-slate-400' }, 'কোনো রিচার্জ ইতিহাস নেই')) :
                  userRecharges.map(rc => React.createElement('tr', { key: rc.rechargeId, className: 'border-b hover:bg-slate-50' },
                    ...
                    React.createElement('td', { className: 'p-2 text-xs text-slate-500' }, new Date(rc.createdAt).toLocaleString()),
      ...
      currentScreen === 'home' && React.createElement('div', { className: 'space-y-6' },
        React.createElement('div', { className: 'text-center' },
          React.createElement('h2', { className: 'text-xl font-bold text-[#0a2e1f]' }, 'আমাদের সেবাসমূহ')
        ),
        React.createElement('div', { className: 'grid grid-cols-2 md:grid-cols-4 gap-4' },
          Object.keys(parentGroups).map(gid => 
            React.createElement('div', { key: gid, onClick: () => openParentGroup(gid), className: 'service-card' },
              React.createElement('div', { className: 'service-card-icon' }, getIcon(parentGroups[gid].icon)),
        ...
        React.createElement('div', { className: 'grid grid-cols-2 md:grid-cols-4 gap-4' },
          allServices.map(svc => 
            React.createElement('div', { key: svc.id, onClick: () => openDirectService(svc.id), className: 'service-card' },
              React.createElement('div', { className: 'service-card-icon' }, getIcon(svc.icon)),
      ...
      currentScreen === 'subservices' && selectedParentGroupId && React.createElement('div', { className: 'space-y-6' },
        ...
        subGroupOrdersList.length > 0 && React.createElement('div', { className: 'bg-white p-4 rounded-xl shadow-md' },
          React.createElement('h3', { className: 'font-bold mb-3' }, 'আপনার পূর্ববর্তী অর্ডার'),
          React.createElement('div', { className: 'overflow-x-auto' },
            React.createElement('table', { className: 'w-full text-sm' },
              React.createElement('thead', null,
                React.createElement('tr', { className: 'bg-slate-50' },
                  React.createElement('th', { className: 'p-2 text-left' }, 'সার্ভিস'),
                  React.createElement('th', { className: 'p-2 text-left' }, 'মূল্য'),
                  React.createElement('th', { className: 'p-2 text-left' }, 'স্ট্যাটাস'),
                  React.createElement('th', { className: 'p-2 text-left' }, 'ডেলিভারি')
                )
              ),
              React.createElement('tbody', null,
                subGroupOrdersList.map(o => 
                  React.createElement('tr', { key: o.orderId, className: 'border-b' },
                    React.createElement('td', { className: 'p-2' }, serviceForms[o.serviceId]?.name),
                    React.createElement('td', { className: 'p-2' }, `৳${o.amount}`),
                    React.createElement('td', { className: 'p-2' }, 
                      o.status === 'pending' ? '⏳ পেন্ডিং' : o.status === 'delivered' ? '✅ সম্পন্ন' : '❌ বাতিল'
                    ),
                    React.createElement('td', { className: 'p-2' },
                      o.deliveryLink ? React.createElement('a', { href: o.deliveryLink, target: '_blank', className: 'text-blue-600 text-xs' }, 'ডাউনলোড') :
                      o.deliveryText ? React.createElement('button', { onClick: () => openDeliveryMessageModal(o.deliveryText), className: 'text-emerald-600 text-xs' }, 'মেসেজ দেখ') : '-'
                    )
                  )
                )
              )
      ...
      // DIRECT SERVICE
      currentScreen === 'direct_service' && selectedServiceId && React.createElement('div', { className: 'space-y-6' },
        React.createElement('div', { className: 'flex items-center justify-between border-b pb-3' },
          React.createElement('h2, { className: 'text-lg font-bold text-[#0a2e1f]' }, serviceForms[selectedServiceId]?.name),
          React.createElement('button', { onClick: () => setCurrentScreen('home'), className: 'text-emerald-600 text-sm' }, '← পেছনে')
        ),
        React.createElement('form', { onSubmit: e => handlePlaceOrder(e, selectedServiceId), className: 'bg-white p-5 rounded-xl shadow-md space-y-4 max-w-2xl mx-auto' },
          serviceForms[selectedServiceId].fields.map(f => 
            React.createElement('div', { key: f.id, className: 'space-y-1' },
              React.createElement('label', { className: 'text-xs font-bold text-slate-600' }, f.name),
              React.createElement('input', { type: f.type || 'text', placeholder: f.name, value: formData[f.id] || '', onChange: e => setFormData({ ...formData, [f.id]: e.target.value }), className: 'w-full p-2 border rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none', required: f.required })
      ...
      currentScreen === 'admin' && React.createElement('div', { className: 'space-y-6' },
        ...
        adminActiveTab === 'orders' && React.createElement('div', { className: 'bg-white p-4 rounded-2xl shadow-md' },
          React.createElement('div', { className: 'flex justify-between items-center mb-4' },
            React.createElement('h3, { className: 'font-bold' }, 'সব অর্ডার'),
            React.createElement('button', { onClick: loadAdminOrders, className: 'text-xs text-emerald-600' }, '↻ রিফ্রেশ')
          ),
          React.createElement('div', { className: 'overflow-x-auto' },
            React.createElement('table', { className: 'w-full text-sm' },
              React.createElement('thead', null,
              ...
              React.createElement('tbody', null,
                adminOrders.map(o => 
                  React.createElement('tr', { key: o.orderId, className: 'border-b hover:bg-slate-50' },
                    ...
                    React.createElement('td', { className: 'p-2' }, `৳${o.amount}`),
                    React.createElement('td', { className: 'p-2' }, 
                      o.status === 'pending' ? React.createElement('span', { className: 'bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full text-xs' }, 'পেন্ডিং') :
                      o.status === 'delivered' ? React.createElement('span', { className: 'bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full text-xs' }, 'ডেলিভারড') :
                      React.createElement('span', { className: 'bg-red-100 text-red-700 px-2 py-0.5 rounded-full text-xs' }, 'বাতিল')
                    ),
                    React.createElement('td', { className: 'p-2' },
                      React.createElement('button', { onClick: () => openOrderDetailsModal(o.formData), className: 'text-blue-600 text-xs mr-2' }, 'দেখুন'),
        ...
        adminActiveTab === 'users' && React.createElement('div', { className: 'bg-white p-4 rounded-2xl shadow-md' },
          React.createElement('div', { className: 'flex justify-between items-center mb-4' },
            React.createElement('h3, { className: 'font-bold' }, 'সব ইউজার'),
            React.createElement('button', { onClick: loadAdminUsers, className: 'text-xs text-emerald-600' }, '↻ রিফ্রেশ')
          ),
          React.createElement('div', { className: 'overflow-x-auto' },
            React.createElement('table', { className: 'w-full text-sm' },
              React.createElement('thead', null,
                React.createElement('tr', { className: 'bg-slate-100' },
                  React.createElement('th', { className: 'p-2 text-left' }, 'ID'),
                  React.createElement('th', { className: 'p-2 text-left' }, 'নাম'),
                  React.createElement('th', { className: 'p-2 text-left' }, 'মোবাইল'),
                  React.createElement('th', { className: 'p-2 text-left' }, 'ব্যালেন্স'),
                  React.createElement('th', { className: 'p-2 text-left' }, 'স্ট্যাটাস'),
                  React.createElement('th', { className: 'p-2 text-left' }, 'অ্যাকশন')
                )
              ),
              React.createElement('tbody', null,
                adminUsers.filter(u => u.role !== 'admin').map(u => 
                  React.createElement('tr', { key: u.userId, className: 'border-b hover:bg-slate-50' },
                    React.createElement('td', { className: 'p-2' }, u.userId),
                    React.createElement('td', { className: 'p-2 font-medium' }, u.name),
                    React.createElement('td', { className: 'p-2' }, u.phone),
                    React.createElement('td', { className: 'p-2 font-bold text-emerald-700' }, `৳${u.balance || 0}`),
                    React.createElement('td', { className: 'p-2' }, u.isBlocked ? React.createElement('span, { className: 'text-red-600' }, 'ব্লকড') : React.createElement('span, { className: 'text-emerald-600' }, 'সক্রিয়')),
                    React.createElement('td', { className: 'p-2' },
                      React.createElement('button', { onClick: () => handleToggleBlockUser(u.userId, u.isBlocked), className: `${u.isBlocked ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-red-600 hover:bg-red-700'} text-white px-3 py-1 rounded text-xs` }, u.isBlocked ? 'আনব্লক' : 'ব্লক')
                    )
                  )
                )
              )
        ...
        adminActiveTab === 'recharges' && React.createElement('div', { className: 'bg-white p-4 rounded-2xl shadow-md' },
          React.createElement('div', { className: 'flex justify-between items-center mb-4' },
            React.createElement('h3, { className: 'font-bold' }, 'রিচার্জ রিকোয়েস্টসমূহ'),
            React.createElement('button', { onClick: loadAdminRecharges, className: 'text-xs text-emerald-600' }, '↻ রিফ্রেশ')
          ),
          React.createElement('div', { className: 'overflow-x-auto' },
            React.createElement('table', { className: 'w-full text-sm' },
              React.createElement('thead', null,
              ...
              React.createElement('tbody', null,
                adminRecharges.map(rc => 
                  React.createElement('tr', { key: rc.rechargeId, className: 'border-b hover:bg-slate-50' },
                    ...
                    React.createElement('td', { className: 'p-2' },
                      rc.status === 'pending' && React.createElement(React.Fragment, null,
