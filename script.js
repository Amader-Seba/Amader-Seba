...
// Complete service forms with rates
const serviceForms = {
...
function App() {
  ...
  // Get rate for a service
  const getServiceRate = (serviceId) => {
    if (serviceForms[serviceId]) return serviceForms[serviceId].rate;
    const directService = allServices.find(s => s.id === serviceId);
    return directService ? directService.rate : 0;
  };
  ...
  return React.createElement('div', { className: 'min-h-screen bg-slate-50' },
    ...
    React.createElement('main', { className: 'max-w-7xl mx-auto px-4 py-8' },
            ...
            React.createElement('table', { className: 'order-history-table' },
              React.createElement('thead', null,
                React.createElement('tr',
                  ...
                  React.createElement('th', null, 'স্ট্যাটাস')
                )
              ),
              React.createElement('tbody', null, 
                userRecharges.length === 0 ? 
                  React.createElement('tr', null, React.createElement('td', { colSpan: 4, className: 'p-3 text-center text-slate-400' }, 'কোনো রিচার্জ ইতিহাস নেই')) :
                  userRecharges.map(rc => React.createElement('tr', { key: rc.rechargeId },
                    React.createElement('td', null, '#' + rc.rechargeId),
                    React.createElement('td', { className: 'font-bold text-emerald-700' }, '৳' + rc.amount),
                    React.createElement('td', { className: 'font-mono text-xs' }, rc.transactionId),
                    React.createElement('td', null, 
                      rc.status === 'pending' ? React.createElement('span', { className: 'order-status-pending' }, '⏳ পেন্ডিং') :
                      rc.status === 'approved' ? React.createElement('span', { className: 'order-status-delivered' }, '✅ অনুমোদিত') :
                      React.createElement('span', { className: 'order-status-cancelled' }, '❌ ব্যর্থ')
                    )
                  ))
              )
            )
        ...
        React.createElement('div', { className: 'grid grid-cols-2 md:grid-cols-5 gap-4' },
          allServices.map(svc => 
            React.createElement('div', { key: svc.id, onClick: () => openDirectService(svc.id), className: 'service-card' },
              ...
              React.createElement('div', { className: 'text-xs mt-1 text-amber-300 font-bold' }, '💰 ' + svc.rate + ' টাকা')
            )
          )
        )
      ),
      
      // SUB SERVICES with checkmark on selected card & order history
      currentScreen === 'subservices' && selectedParentGroupId && React.createElement('div', { className: 'space-y-6 animate-fade-in' },
        React.createElement('div', { className: 'flex items-center justify-between border-b pb-3' },
          ...
          React.createElement('button', { onClick: () => { setCurrentScreen('home'); window.history.pushState({}, ''); }, className: 'text-emerald-600 text-sm hover:underline' }, '← পেছনে')
        ),
        React.createElement('div', { className: 'grid grid-cols-3 gap-2' }, 
          parentGroups[selectedParentGroupId].children.map(cid => {
            ...
            return React.createElement('div', { 
              ...
              isSelected && React.createElement('div', { className: 'check-mark' }, '✓'),
              React.createElement('div', { className: 'font-semibold text-sm text-white' }, serviceForms[cid]?.name),
              React.createElement('div', { className: 'text-xs mt-1 text-amber-300 font-bold' }, '💰 ' + serviceForms[cid]?.rate + ' টাকা')
            );
          })
        ),
        selectedServiceId && serviceForms[selectedServiceId] && React.createElement('div', { className: 'bg-white p-5 rounded-xl shadow-md space-y-4' },
          React.createElement('div', { className: 'bg-amber-50 border border-amber-200 rounded-lg p-2 text-center mb-2' },
            React.createElement('span', { className: 'text-amber-800 font-bold text-sm' }, '💵 এই সার্ভিসের জন্য খরচ হবে: ', React.createElement('span', { className: 'text-emerald-700 text-base' }, '৳' + serviceForms[selectedServiceId].rate, ' টাকা'))
          ),
          React.createElement('form', { onSubmit: e => handlePlaceOrder(e, selectedServiceId), className: 'space-y-4' },
            ...
            React.createElement('button', { 
              ...
              className: 'w-full bg-emerald-600 hover:bg-emerald-700 py-2 rounded-lg text-white font-bold transition flex items-center justify-center gap-2'
            }, 
              orderSubmitLoading ? React.createElement('div', { className: 'w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin' }) : 
              orderSubmitSuccess ? React.createElement('span', { className: 'check-animation' }, '✓ অর্ডার সফল!') : 'অর্ডার সাবমিট করুন'
            )
          )
        ),
        // Order History Table
        React.createElement('div', { className: 'bg-white p-2 rounded-xl shadow-md mt-4' },
          React.createElement('h3', { className: 'font-bold text-slate-800 mb-2 text-xs' }, '📋 আপনার অর্ডার ইতিহাস'),
          React.createElement('div', { className: 'overflow-x-auto' },
            React.createElement('table', { className: 'order-history-table' },
              React.createElement('thead', null,
                React.createElement('tr',
                  React.createElement('th', null, 'ক্রমিক'),
                  React.createElement('th', null, 'সার্ভিস'),
                  React.createElement('th', null, 'তথ্য'),
                  React.createElement('th', null, 'মূল্য'),
                  React.createElement('th', null, 'স্ট্যাটাস'),
                  React.createElement('th', null, 'ডেলিভারি')
                )
              ),
              React.createElement('tbody', null,
                subGroupOrdersList.length === 0 ? 
                  React.createElement('tr', null, React.createElement('td', { colSpan: 6, className: 'p-3 text-center text-slate-400 text-xs' }, 'কোনো অর্ডার ইতিহাস নেই')) :
                  subGroupOrdersList.map((o, idx) => {
                    const meta = serviceForms[o.serviceId];
                    return React.createElement('tr', { key: o.orderId },
                      React.createElement('td', { className: 'text-center' }, idx + 1),
                      React.createElement('td', { className: 'font-medium text-slate-700' }, meta?.name || o.serviceId),
                      React.createElement('td', null, React.createElement('button', { onClick: () => openOrderDetailsModal(o.formData), className: 'order-view-btn' }, '👁️ তথ্য')),
                      React.createElement('td', { className: 'font-bold text-emerald-700' }, '৳' + o.amount),
                      React.createElement('td', null, 
                        o.status === 'pending' ? React.createElement('span', { className: 'order-status-pending' }, '⏳ পেন্ডিং') :
                        o.status === 'delivered' ? React.createElement('span', { className: 'order-status-delivered' }, '✅ সম্পন্ন') :
                        React.createElement('span', { className: 'order-status-cancelled' }, '❌ বাতিল')
                      ),
                      React.createElement('td', null,
                        o.status === 'delivered' ? (
                          o.deliveryLink ? React.createElement('a', { href: o.deliveryLink, target: '_blank', className: 'order-download-btn' }, '📥 ডাউনলোড') :
                          o.deliveryText ? React.createElement('button', { onClick: () => openDeliveryMessageModal(o.deliveryText), className: 'order-message-btn' }, '💬 মেসেজ') :
                          React.createElement('span', { className: 'text-slate-400 text-xs' }, 'ডেলিভারড')
                        ) : React.createElement('span', { className: 'text-slate-400 text-xs' }, 'অপেক্ষমান')
                      )
                    );
                  })
              )
      ...
      // DIRECT SERVICE with order history and rate display
      currentScreen === 'direct_service' && selectedServiceId && React.createElement('div', { className: 'space-y-6 animate-fade-in' },
        React.createElement('div', { className: 'flex items-center justify-between border-b pb-3' },
          React.createElement('h2', { className: 'text-lg font-bold text-[#0a2e1f]' }, serviceForms[selectedServiceId]?.name),
          React.createElement('button', { onClick: () => { setCurrentScreen('home'); window.history.pushState({}, ''); }, className: 'text-emerald-600 text-sm hover:underline' }, '← পেছনে')
        ),
        React.createElement('div', { className: 'bg-white p-5 rounded-xl shadow-md max-w-2xl mx-auto' },
          React.createElement('div', { className: 'bg-amber-50 border border-amber-200 rounded-lg p-2 text-center mb-4' },
            React.createElement('span', { className: 'text-amber-800 font-bold text-sm' }, '💵 এই সার্ভিসের জন্য খরচ হবে: ', React.createElement('span', { className: 'text-emerald-700 text-base' }, '৳' + serviceForms[selectedServiceId].rate, ' টাকা'))
          ),
          React.createElement('form', { onSubmit: e => handlePlaceOrder(e, selectedServiceId), className: 'space-y-4' },
            serviceForms[selectedServiceId].fields.map(f => 
      ...
      currentScreen === 'admin' && React.createElement('div', { className: 'space-y-6' },
        ...
        adminActiveTab === 'recharges' && React.createElement('div', { className: 'bg-white p-4 rounded-2xl shadow-md' },
          React.createElement('div', { className: 'flex justify-between items-center mb-4' }, React.createElement('h3', { className: 'font-bold' }, 'রিচার্জ রিকোয়েস্ট'), React.createElement('button', { onClick: loadAdminRecharges, className: 'text-xs text-emerald-600' }, '↻ রিফ্রেশ')),
          React.createElement('div', { className: 'overflow-x-auto' },
            React.createElement('table', { className: 'order-history-table' },
              React.createElement('thead', null, React.createElement('tr', 
                React.createElement('th', null, 'ID'), React.createElement('th', null, 'ইউজার'), React.createElement('th', null, 'পরিমাণ'), React.createElement('th', null, 'Txn ID'), React.createElement('th', null, 'স্ট্যাটাস'), React.createElement('th', null, 'অ্যাকশন')
              )),
              React.createElement('tbody', null, adminRecharges.map(rc => 
                React.createElement('tr', { key: rc.rechargeId },
                  ...
                  React.createElement('td', { className: 'font-bold' }, '৳' + rc.amount),
                  React.createElement('td', { className: 'font-mono text-xs' }, rc.transactionId),
                  React.createElement('td', null, 
                    rc.status === 'pending' ? React.createElement('span', { className: 'order-status-pending' }, 'পেন্ডিং') :
                    rc.status === 'approved' ? React.createElement('span', { className: 'order-status-delivered' }, 'অনুমোদিত') :
                  ...
                  React.createElement('td', null, rc.status === 'pending' && React.createElement(React.Fragment, null,
                    React.createElement('button', { onClick: () => handleUpdateRechargeStatus(rc.rechargeId, 'approved'), className: 'order-download-btn mr-1' }, 'অনুমোদন'),
                    React.createElement('button', { onClick: () => handleUpdateRechargeStatus(rc.rechargeId, 'failed'), className: 'order-status-cancelled' }, 'রিজেক্ট')
                  ))
                )
              ))
            )
        ...
        adminActiveTab === 'notice' && React.createElement('div', { className: 'bg-white p-6 rounded-2xl shadow-md max-w-lg mx-auto' },
          ...
          React.createElement('button', { onClick: handleUpdateNoticeSubmit, className: 'mt-4 w-full bg-emerald-700 hover:bg-emerald-600 py-2 rounded-full text-white font-bold transition' }, 'নোটিশ আপডেট করুন')
        )
      )
    ),
    ...
    modalOpen && React.createElement('div', { className: 'fixed inset-0 bg-black/50 flex justify-center items-center z-[9999]' },
      React.createElement('div', { className: 'bg-white p-6 rounded-2xl max-w-md w-full mx-4' },
        ...
        React.createElement('button', { onClick: () => setModalOpen(false), className: 'mt-4 bg-slate-200 hover:bg-slate-300 px-4 py-2 rounded-lg w-full font-medium transition' }, 'বন্ধ করুন')
      )
    ),
    ...
    deliveryModalOpen && React.createElement('div', { className: 'fixed inset-0 bg-black/50 flex justify-center items-center z-[9999]' },
      React.createElement('div', { className: 'bg-white p-6 rounded-2xl w-96 max-w-full mx-4' },
        ...
        React.createElement('div', { className: 'flex gap-3 mt-5' },
          React.createElement('button', { onClick: submitCustomDelivery, className: 'flex-1 bg-emerald-600 hover:bg-emerald-700 py-2 rounded-lg text-white font-bold transition' }, 'জমা দিন'),
          React.createElement('button', { onClick: () => setDeliveryModalOpen(false), className: 'flex-1 bg-slate-200 hover:bg-slate-300 py-2 rounded-lg font-medium transition' }, 'বাতিল')
        )
      )
    )
  );
