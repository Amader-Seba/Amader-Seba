// ==================== কনফিগ ====================
const API_URL = 'https://script.google.com/macros/s/AKfycbyJ12GcsAKJ1pBfiFMpHHwItbJsH0U-Fu-Pd1ZF8Ag_UAa_WF574mS2prBu_QWov5Qc/exec'; // এখানে বসান ডিপ্লয়মেন্ট URL
...
const serviceForms = {
  nid_number: {
    ...
    fields: [{name:'নাম *', id:'name', required:true},{name:'এনআইডি নম্বর *', id:'nid', required:true},{name:'জন্ম তারিখ', id:'dob', type:'date'}]
  },
  voter_slip: {
    name: 'ভোটার স্লিপ নম্বর', icon: 'fa-id-card', parent: 'এনআইডি কার্ড সার্ভিস', rate: 350,
    fields: [{name:'নাম *', id:'name', required:true},{name:'ভোটার স্লিপ নম্বর *', id:'voter_slip', required:true},{name:'জন্ম তারিখ', id:'dob', type:'date'}]
  },
  voter_number: {
    name: 'ভোটার নম্বর', icon: 'fa-id-card', parent: 'এনআইডি কার্ড সার্ভিস', rate: 400,
    fields: [{name:'নাম *', id:'name', required:true},{name:'ভোটার নম্বর *', id:'voter_no', required:true},{name:'জন্ম তারিখ', id:'dob', type:'date'}]
  },
  server_copy: {
    name: 'সার্ভার কপি', icon: 'fa-id-card', parent: 'এনআইডি কার্ড সার্ভিস', rate: 150,
    fields: [{name:'এনআইডি নম্বর *', id:'nid', required:true},{name:'জন্ম তারিখ *', id:'dob', type:'date', required:true}]
  },
  smart_card: {
  ...
  lost_id: {
    ...
    fields: [{name:'নাম *', id:'name', required:true},{name:'পিতার নাম *', id:'father', required:true},{name:'মাতার নাম *', id:'mother', required:true},{name:'জন্ম নিবন্ধন নাম্বার', id:'birth_reg_no'},{name:'বিভাগের নাম *', id:'division', required:true},{name:'জেলার নাম *', id:'district', required:true},{name:'উপজেলার নাম *', id:'upazila', required:true},{name:'ইউনিয়নের নাম *', id:'union', required:true},{name:'ওয়ার্ড নম্বর *', id:'ward', required:true},{name:'গ্রামের নাম *', id:'village', required:true}]
  },
  nid_to_sim: {
  ...
  imei_active: {
    ...
    fields: [{name:'IMEI 1 *', id:'imei1', required:true},{name:'IMEI 2', id:'imei2'},{name:'হারানোর তারিখ *', id:'lost_date', type:'date', required:true},{name:'সর্বশেষ ব্যবহৃত মোবাইল নং *', id:'last_number', required:true}]
  },
  lost_nid_form_nid: {
  ...
  lost_nid_form_user: {
    ...
    fields: [{name:'ইউজার নেম *', id:'username', required:true},{name:'পাসওয়ার্ড *', id:'password', type:'password', required:true}]
  },
  birth_normal: {
    name: 'নরমাল নিবন্ধন', icon: 'fa-baby', parent: 'নতুন জন্ম নিবন্ধন সার্ভিস', rate: 2400,
    fields: [{section:'ব্যক্তিগত তথy'},{name:'নাম (বাংলা) *', id:'name_bn', required:true},{name:'নাম (ইংরেজি) *', id:'name_en', required:true},{name:'লিঙ্গ *', id:'gender', type:'select', options:['পুরুষ','মহিলা'], required:true},{name:'জন্ম তারিখ *', id:'dob', type:'date', required:true},{name:'জন্মস্থান *', id:'birth_place', required:true},{section:'পিতার তথ্য'},{name:'পিতার নাম (বাংলা) *', id:'father_bn', required:true},{name:'পিতার নাম (ইংরেজি) *', id:'father_en', required:true},{section:'মাতার তথ্য'},{name:'মাতার নাম (বাংলা) *', id:'mother_bn', required:true},{name:'মাতার নাম (ইংরেজি) *', id:'mother_en', required:true},{name:'মোবাইল নম্বর *', id:'mobile', required:true},{section:'স্থায়ী ঠিকানা'},{name:'বিভাগ *', id:'division', required:true},{name:'জেলা *', id:'district', required:true},{name:'উপজেলা *', id:'upazila', required:true},{name:'ইউনিয়ন *', id:'union', required:true},{name:'পোস্ট অফিস *', id:'post_office', required:true},{name:'ওয়ার্ড নম্বর *', id:'ward', required:true},{name:'গ্রাম *', id:'village', required:true}]
  },
  birth_minister: {
    name: 'মিনিস্টার নিবন্ধন', icon: 'fa-baby', parent: 'নতুন জন্ম নিবন্ধন সার্ভিস', rate: 3200,
    fields: [{section:'ব্যক্তিগত তথ্য'},{name:'নাম (বাংলা) *', id:'name_bn', required:true},{name:'নাম (ইংরেজি) *', id:'name_en', required:true},{name:'লিঙ্গ *', id:'gender', type:'select', options:['পুরুষ','মহিলা'], required:true},{name:'জন্ম তারিখ *', id:'dob', type:'date', required:true},{name:'জন্মস্থান *', id:'birth_place', required:true},{section:'পিতার তথ্য'},{name:'পিতার নাম (বাংলা) *', id:'father_bn', required:true},{name:'পিতার নাম (ইংরেজি) *', id:'father_en', required:true},{section:'মাতার তথ্য'},{name:'মাতার নাম (বাংলা) *', id:'mother_bn', required:true},{name:'মাতার নাম (ইংরেজি) *', id:'mother_en', required:true},{name:'মোবাইল নম্বর *', id:'mobile', required:true},{section:'স্থায়ী ঠিকানা'},{name:'বিভাগ *', id:'division', required:true},{name:'জেলা *', id:'district', required:true},{name:'উপজেলা *', id:'upazila', required:true},{name:'ইউনিয়ন *', id:'union', required:true},{name:'পোস্ট অফিস *', id:'post_office', required:true},{name:'ওয়ার্ড নম্বর *', id:'ward', required:true},{name:'গ্রাম *', id:'village', required:true}]
  }
};
...
async function doLogin(){
  ...
  if(res.success){
    ...
    setCookie('userPhone',res.phone,7); setCookie('userRole',res.role||'user',7);
    updateNav();
...
function logout(){
  currentUser=null;
  ['userId','userName','userPhone','userRole','adminLogged'].forEach(c=>setCookie(c,'',-1));
  updateNav(); renderLandingPage();
}
function updateNav(){
  ...
  if(currentUser){
    ...
    greet.style.display='none'; homeBtn.style.display='none'; myOrdersBtn.style.display='none';
    loginBtn.style.display='inline-block'; registerBtn.style.display='inline-block'; logoutBtn.style.display='none';
  }
}

// ==================== ল্যান্ডিং পেজ (লগইনহীন - শুধু কার্টুন) ====================
function renderLandingPage(){
  let html = `<div style="text-align:center;padding:20px 0">
    ...
    <div class="landing-grid">
      <div class="landing-card"><i class="fas fa-id-card"></i><h4>আইডি কার্ড</h4></div>
      <div class="landing-card"><i class="fas fa-sim-card"></i><h4>সিম বায়োমেট্রিক</h4></div>
      <div class="landing-card"><i class="fas fa-map-marker-alt"></i><h4>লোকেশন সার্ভিস</h4></div>
      <div class="landing-card"><i class="fas fa-search"></i><h4>হারানো আইডি</h4></div>
      <div class="landing-card"><i class="fas fa-link"></i><h4>এনআইডি টু সিম</h4></div>
      <div class="landing-card"><i class="fas fa-file-invoice"></i><h4>ই-টিন</h4></div>
      <div class="landing-card"><i class="fas fa-mobile-alt"></i><h4>IMEI টু নম্বর</h4></div>
      <div class="landing-card"><i class="fas fa-file-alt"></i><h4>হারানো এনআইডি ফরম</h4></div>
      <div class="landing-card"><i class="fas fa-baby"></i><h4>জন্ম নিবন্ধন</h4></div>
    </div>
...
function renderSubServices(parentId, title, childIds){
  ...
  childIds.forEach(sid=>{
    ...
    if(!s) return;
    const card=document.createElement('div'); card.className='service-card';
    card.innerHTML=`<i class="fas ${s.icon}"></i><h4>${s.name}</h4><div class="price">${s.rate} টাকা</div><span class="checkmark"><i class="fas fa-check"></i></span>`;
    card.onclick=()=>{
...
async function submitOrder(e,serviceId){
  ...
  for(let f of s.fields){
    if(f.required && !formData[f.id] && !f.section){
...
async function adminTab(tab){
  ...
  if(tab==='dashboard'){
    ...
    let html='<table><thead><tr><th>ID</th><th>নাম</th><th>ফোন</th><th>হোয়াটসঅ্যাপ</th><th>রোল</th><th>স্ট্যাটাস</th><th>অ্যাকশন</th></tr></thead><tbody>';
    (res.users||[]).forEach(u=>{
    ...
    html+='</tbody></td>'; content.innerHTML=html;
  }
}
...
// ==================== লগইন/রেজিস্টার ফর্ম (অ্যাডমিন আলাদা নাই) ====================
function showLoginForm(){
  $('app').innerHTML=`<h2 class="page-title">লগইন</h2>
    ...
    <p style="margin-top:8px;font-size:0.8rem;color:#666;">অ্যাডমিন: আপনার শীটে Role='admin' যুক্ত অ্যাকাউন্ট দিয়ে লগইন করুন</p>`;
}
...
// ==================== ইনিশিয়াল ====================
(async function init(){
  const uid=getCookie('userId'), uname=getCookie('userName'), uphone=getCookie('userPhone'), urole=getCookie('userRole');
  if(uid) currentUser={userId:uid,name:uname,phone:uphone,role:urole||'user'};
  updateNav();
  if(currentUser?.role==='admin') renderAdminPanel();
  else if(currentUser) renderHome();
  else renderLandingPage();
  $('homeBtn').addEventListener('click',()=>{if(currentUser) renderHome(); else renderLandingPage();});
