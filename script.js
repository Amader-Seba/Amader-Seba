...
const allServices = [
  { id: 'sim_location', name: 'সিম নাম্বার লোকেশন', icon: '📍' },
  { id: 'lost_id', name: 'হারানো আইডি কার্ড', icon: '🔍' },
  { id: 'nid_to_sim', name: 'এনআইডি টু সকল সিম', icon: '📤' },
  { id: 'lost_etin', name: 'হারানো ই-টিন', icon: '📄' },
  { id: 'imei_active', name: 'IMEI To Active', icon: '📱' }
];
...
function App() {
  ...
  return React.createElement('div', { className: 'min-h-screen bg-slate-50' },
    ...
    React.createElement('main', { className: 'max-w-7xl mx-auto px-4 py-8' },
                ...
                userRecharges.length === 0 ? 
                  ...
                  userRecharges.map(rc => React.createElement('tr', { key: rc.rechargeId, className: 'border-b hover:bg-slate-50' },
                    ...
                    React.createElement('td', { className: 'p-2' }, 
                      rc.status === 'pending' ? React.createElement('span', { className: 'badge-pending' }, '⏳ পেন্ডিং') :
                      rc.status === 'approved' ? React.createElement('span', { className: 'badge-delivered' }, '✅ অনুমোদিত') :
                      React.createElement('span', { className: 'badge-cancelled' }, '❌ ব্যর্থ')
                    )
                  ))
              )
            )
      ...
      // HOME - Services Grid with checkmark on selected
      ...
      // SUB SERVICES with checkmark on selected card
      currentScreen === 'subservices' && selectedParentGroupId && React.createElement('div', { className: 'space-y-6 animate-fade-in' },
        ...
        React.createElement('div', { className: 'grid grid-cols-3 gap-2' }, 
          parentGroups[selectedParentGroupId].children.map(cid => {
            ...
            return React.createElement('div', { 
              ...
              React.createElement('div', { className: 'text-xs mt-1 text-emerald-300' }, '💰 ' + serviceForms[cid]?.rate + ' টাকা')
            );
          })
        ),
        ...
        // Order History Table
        React.createElement('div', { className: 'bg-white p-4 rounded-xl shadow-md mt-6' },
          React.createElement('h3', { className: 'font-bold text-slate-800 mb-3' }, '📋 আপনার অর্ডার ইতিহাস'),
          React.createElement('div', { className: 'overflow-x-auto' },
            React.createElement('table', { className: 'order-table' },
              React.createElement('thead', null,
                React.createElement('tr',
                  React.createElement('th', null, 'ক্রমিক'),
                  React.createElement('th', null, 'সার্ভিসের নাম'),
                  React.createElement('th', null, 'তথ্য বিবরণী'),
                  React.createElement('th', null, 'মূল্য'),
                  React.createElement('th', null, 'স্ট্যাটাস'),
                  React.createElement('th', null, 'ডেলিভারি মেসেজ / ডাউনলোড')
                )
              ),
              React.createElement('tbody', null,
                subGroupOrdersList.length === 0 ? 
                  React.createElement('tr', null, React.createElement('td', { colSpan: 6, className: 'p-4 text-center text-slate-400' }, 'কোনো অর্ডার ইতিহাস নেই')) :
                  subGroupOrdersList.map((o, idx) => {
                    ...
                    return React.createElement('tr', { key: o.orderId },
                      ...
                      React.createElement('td', { className: 'font-medium' }, meta?.name || o.serviceId),
                      React.createElement('td', null, React.createElement('button', { onClick: () => openOrderDetailsModal(o.formData), className: 'text-blue-600 text-xs hover:underline' }, 'তথ্য দেখুন')),
                      React.createElement('td', null, '৳' + o.amount),
                      React.createElement('td', null, 
                        o.status === 'pending' ? React.createElement('span', { className: 'badge-pending' }, 'পেন্ডিং') :
                        o.status === 'delivered' ? React.createElement('span', { className: 'badge-delivered' }, 'সম্পন্ন') :
                        React.createElement('span', { className: 'badge-cancelled' }, 'বাতিল')
                      ),
                      React.createElement('td', null,
                        o.status === 'delivered' ? (
                          o.deliveryLink ? React.createElement('a', { href: o.deliveryLink, target: '_blank', className: 'text-emerald-600 text-sm hover:underline' }, 'ডাউনলোড') :
                          o.deliveryText ? React.createElement('button', { onClick: () => openDeliveryMessageModal(o.deliveryText), className: 'text-emerald-600 text-sm hover:underline' }, 'মেসেজ দেখ') :
      ...
      // DIRECT SERVICE with order history
              ...
              React.createElement('tbody', null,
                subGroupOrdersList.length === 0 ? 
                  ...
                  subGroupOrdersList.map((o, idx) => {
                    ...
                    return React.createElement('tr', { key: o.orderId },
                      React.createElement('td', null, idx + 1),
                      React.createElement('td', null, meta?.name || o.serviceId),
                      React.createElement('td', null, React.createElement('button', { onClick: () => openOrderDetailsModal(o.formData), className: 'text-blue-600 text-xs hover:underline' }, 'তথ্য দেখুন')),
                      React.createElement('td', null, '৳' + o.amount),
                      React.createElement('td', null, 
                        o.status === 'pending' ? React.createElement('span', { className: 'badge-pending' }, 'পেন্ডিং') :
                      ...
                      React.createElement('td', null,
                        o.status === 'delivered' ? (
                          o.deliveryLink ? React.createElement('a', { href: o.deliveryLink, target: '_blank', className: 'text-emerald-600 text-sm' }, 'ডাউনলোড') :
                          o.deliveryText ? React.createElement('button', { onClick: () => openDeliveryMessageModal(o.deliveryText), className: 'text-emerald-600 text-sm' }, 'মেসেজ') : '-'
                        ) : '-'
                      )
                    );
                  })
      ...
      // ADMIN PANEL (simplified but complete)
            ...
            React.createElement('table', { className: 'w-full text-sm' },
              ...
              React.createElement('tbody', null, adminRecharges.map(rc => 
                React.createElement('tr', { key: rc.rechargeId, className: 'border-b hover:bg-slate-50' },
                  ...
                  React.createElement('td', { className: 'p-2 font-mono text-xs' }, rc.transactionId),
                  React.createElement('td', { className: 'p-2' }, rc.status === 'pending' ? React.createElement('span', { className: 'badge-pending' }, 'পেন্ডিং') : (rc.status === 'approved' ? React.createElement('span', { className: 'badge-delivered' }, 'অনুমোদিত') : React.createElement('span', { className: 'badge-cancelled' }, 'ব্যর্থ'))),
                  React.createElement('td', { className: 'p-2' }, rc.status === 'pending' && React.createElement(React.Fragment, null, 
                    React.createElement('button', { onClick: () => handleUpdateRechargeStatus(rc.rechargeId, 'approved'), className: 'bg-emerald-600 text-white px-2 py-1 rounded text-xs mr-1 hover:bg-emerald-700' }, 'অনুমোদন'),
                    React.createElement('button', { onClick: () => handleUpdateRechargeStatus(rc.rechargeId, 'failed'), className: 'bg-red-600 text-white px-2 py-1 rounded text-xs hover:bg-red-700' }, 'রিজেক্ট')
                  ))
                )
              ))
            )
