...
function simulateLocalApi(action, p) {
  ...
  if (action === 'getUsers') {
    return { success: true, users: users.map(u => ({ userId: u.userId, name: u.name, phone: u.phone, isBlocked: u.isBlocked, role: u.role || 'user', balance: u.balance ?? 0 })) };
  }
...
const parentGroups = {
  nid_card: { id: 'nid_card', name: 'এনআইডি কার্ড', icon: '🪪', children: ['nid_number', 'voter_slip', 'voter_number', 'server_copy', 'smart_card'] },
  sim_biometric: { id: 'sim_biometric', name: 'সিম বায়োমেট্রিক', icon: '📱', children: ['bl_bio', 'gp_bio', 'robi_bio', 'teletalk_bio'] },
  lost_nid_form: { id: 'lost_nid_form', name: 'হারানো এনআইডি আবেদন ফরম', icon: '📄', children: ['lost_nid_form_nid', 'lost_nid_form_user'] },
  birth_reg: { id: 'birth_reg', name: 'নতুন জন্ম নিবন্ধন', icon: '👶', children: ['birth_normal', 'birth_minister'] }
};

const allServices = [
  { id: 'sim_location', name: 'সিম নাম্বার লোকেশন', icon: '📍', rate: 850 },
  { id: 'lost_id', name: 'হারানো আইডি কার্ড', icon: '🔍', rate: 2000 },
  { id: 'nid_to_sim', name: 'এনআইডি টু সকল সিম', icon: '📤', rate: 900 },
  { id: 'lost_etin', name: 'হারানো ই-টিন', icon: '📄', rate: 200 },
  { id: 'imei_active', name: 'IMEI To Active', icon: '📱', rate: 900 }
];
...
function App() {
  ...
  const [orderSubmitLoading, setOrderSubmitLoading] = React.useState(false);
  const [orderSubmitSuccess, setOrderSubmitSuccess] = React.useState(false);
  ...
  const handleBackButton = () => {
    if (currentScreen === 'subservices') {
      setCurrentScreen('home');
      setSelectedParentGroupId('');
      setSelectedServiceId('');
    } else if (currentScreen === 'direct_service') {
      setCurrentScreen('home');
      setSelectedServiceId('');
  ...
  const handlePlaceOrder = async (e, sid) => {
    ...
    if (res?.success) { 
      ...
      await refreshUserBalance(); 
      setTimeout(() => setOrderSubmitSuccess(false), 2000);
    } else { 
      showToast(res?.message || 'অর্ডার ব্যর্থ');
      setOrderSubmitSuccess(false);
    }
  ...
  // ডেলিভারি মেসেজ পপআপ - কপি বাটন সহ
  const openDeliveryMessageModal = (msg) => { 
    setModalTitle('📨 ডেলিভারি মেসেজ'); 
    setModalType('delivered_message');
    setModalContent(msg); 
    setModalOpen(true); 
  };

  // UI Rendering (শুধু Modal অংশ পরিবর্তন করা হয়েছে - বাকি সব আগের মতো)
  // Modal component with copy button
  const modalContentJSX = modalOpen && React.createElement('div', { className: 'fixed inset-0 bg-black/50 flex justify-center items-center z-[9999]' },
    React.createElement('div', { className: 'bg-white p-4 rounded-xl max-w-md w-full mx-4' },
      React.createElement('h3, { className: 'font-bold text-sm mb-2' }, modalTitle),
      modalType === 'delivered_message' ? 
        React.createElement('div', { className: 'bg-emerald-50 border border-emerald-200 rounded-lg p-3 mb-3' },
          React.createElement('p, { className: 'text-sm text-emerald-900 font-medium whitespace-pre-wrap break-words' }, modalContent),
          React.createElement('button', { 
            onClick: () => copyText(modalContent), 
            className: 'mt-3 w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs py-1.5 px-3 rounded-lg transition flex items-center justify-center gap-2'
          }, 
            React.createElement('span', { className: 'text-sm' }, '📋'), ' মেসেজ কপি করুন'
          )
        ) :
        React.createElement('pre', { className: 'mt-2 text-xs whitespace-pre-wrap bg-slate-50 p-2 rounded-lg max-h-80 overflow-auto' }, modalContent),
      React.createElement('button', { onClick: () => setModalOpen(false), className: 'mt-2 bg-slate-200 hover:bg-slate-300 px-3 py-1.5 rounded-lg w-full font-medium text-sm transition' }, 'বন্ধ করুন')
    )
  );
  ...
  return React.createElement('div', { className: 'min-h-screen bg-slate-50' },
    ...
    // Main Content (শুধু modal কল করা হয়েছে - বাকি আগের মতো)
    React.createElement('main', { className: 'max-w-7xl mx-auto px-4 py-8' },
      ...
      currentScreen === 'landing' && React.createElement('div', { className: 'space-y-8' },
        React.createElement('div', { className: 'rounded-3xl bg-gradient-to-r from-slate-900 to-slate-800 text-white p-10 text-center' },
          ...
          React.createElement('div', { className: 'flex gap-4 justify-center mt-6' },
            React.createElement('button', { onClick: () => navigateTo('login'), className: 'bg-emerald-600 hover:bg-emerald-500 px-8 py-3 rounded-full font-bold transition transform hover:scale-105' }, 'লগইন'),
            React.createElement('button', { onClick: () => navigateTo('register'), className: 'bg-white text-slate-900 hover:bg-slate-100 px-8 py-3 rounded-full font-bold transition transform hover:scale-105' }, 'রেজিস্টার')
          )
        ),
      ...
      // LOGIN
      currentScreen === 'login' && React.createElement('div', { className: 'max-w-md mx-auto bg-white p-8 rounded-3xl shadow-lg animate-fade-in' },
        ...
        React.createElement('form', { onSubmit: handleLogin, className: 'space-y-4 mt-6' },
          React.createElement('input', { type: 'tel', placeholder: 'মোবাইল নম্বর', value: loginPhone, onChange: e => setLoginPhone(e.target.value), className: 'w-full p-3 border rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none transition', required: true }),
          React.createElement('input', { type: 'password', placeholder: 'পাসওয়ার্ড', value: loginPass, onChange: e => setLoginPass(e.target.value), className: 'w-full p-3 border rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none transition', required: true }),
          React.createElement('button', { 
            type: 'submit', 
            disabled: loginLoading,
            className: 'w-full bg-emerald-700 hover:bg-emerald-600 py-3 rounded-full text-white font-bold transition transform hover:scale-105 flex items-center justify-center gap-2'
          }, 
            loginLoading ? React.createElement('div', { className: 'w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin' }) : 'লগইন করুন'
          )
        ),
        React.createElement('p', { className: 'text-center mt-6 text-slate-500' }, 'নতুন? ', React.createElement('button', { onClick: () => navigateTo('register'), className: 'text-emerald-700 font-bold hover:underline' }, 'রেজিস্টার করুন'))
      ),
      
      // REGISTER
      currentScreen === 'register' && React.createElement('div', { className: 'max-w-md mx-auto bg-white p-8 rounded-3xl shadow-lg animate-fade-in' },
        ...
        React.createElement('form', { onSubmit: handleRegister, className: 'space-y-4 mt-6' },
          ...
          React.createElement('button', { type: 'submit', className: 'w-full bg-emerald-700 hover:bg-emerald-600 py-3 rounded-full text-white font-bold transition transform hover:scale-105' }, 'রেজিস্টার')
        ),
        React.createElement('p', { className: 'text-center mt-6 text-slate-500' }, 'ইতিমধ্যে অ্যাকাউন্ট আছে? ', React.createElement('button', { onClick: () => navigateTo('login'), className: 'text-emerald-700 font-bold hover:underline' }, 'লগইন করুন'))
      ),
      
      // RECHARGE
      currentScreen === 'recharge' && currentUser && React.createElement('div', { className: 'max-w-2xl mx-auto space-y-5 animate-fade-in' },
        React.createElement('div', { className: 'bg-gradient-to-r from-amber-500 to-amber-600 rounded-2xl p-5 text-white text-center' },
          ...
          React.createElement('button', { onClick: () => copyText(NAGAD_NUMBER), className: 'bg-amber-700 hover:bg-amber-800 px-4 py-1 rounded-full text-xs mt-2 transition' }, '📋 কপি করুন')
        ),
        React.createElement('form', { onSubmit: handleRechargeSubmit, className: 'bg-white p-5 rounded-xl shadow-md space-y-4' },
          ...
          React.createElement('button', { type: 'submit', className: 'w-full bg-amber-500 hover:bg-amber-600 py-3 rounded-full text-white font-black transition transform hover:scale-105' }, 'রিচার্জ রিকোয়েস্ট পাঠান')
        ),
        React.createElement('div', { className: 'bg-white p-4 rounded-xl shadow-md' },
          React.createElement('div', { className: 'flex justify-between items-center mb-3' },
            ...
            React.createElement('button', { onClick: loadUserRecharges, className: 'text-xs text-emerald-600 hover:text-emerald-700 transition' }, '↻ রিফ্রেশ')
          ),
          React.createElement('div', { className: 'overflow-x-auto' },
            React.createElement('table', { className: 'order-history-table' },
              React.createElement('thead', null,
                React.createElement('tr',
                  React.createElement('th', { className: 'text-[10px] p-1' }, 'ID'),
                  React.createElement('th', { className: 'text-[10px] p-1' }, 'পরিমাণ'),
                  React.createElement('th', { className: 'text-[10px] p-1' }, 'Txn ID'),
                  React.createElement('th', { className: 'text-[10px] p-1' }, 'স্ট্যাটাস')
                )
              ),
              React.createElement('tbody', null, 
                userRecharges.length === 0 ? 
                  React.createElement('tr', null, React.createElement('td', { colSpan: 4, className: 'p-2 text-center text-slate-400 text-[10px]' }, 'কোনো রিচার্জ ইতিহাস নেই')) :
                  userRecharges.map(rc => React.createElement('tr', { key: rc.rechargeId },
                    React.createElement('td', { className: 'text-[10px] p-1' }, '#' + rc.rechargeId),
                    React.createElement('td', { className: 'font-bold text-emerald-700 text-[10px] p-1' }, '৳' + rc.amount),
                    React.createElement('td', { className: 'font-mono text-[9px] p-1' }, rc.transactionId),
                    React.createElement('td', { className: 'p-1' }, 
                      rc.status === 'pending' ? React.createElement('span', { className: 'order-status-pending text-[9px] py-0.5 px-1.5' }, '⏳ পেন্ডিং') :
                      rc.status === 'approved' ? React.createElement('span', { className: 'order-status-delivered text-[9px] py-0.5 px-1.5' }, '✅ অনুমোদিত') :
                      React.createElement('span', { className: 'order-status-cancelled text-[9px] py-0.5 px-1.5' }, '❌ ব্যর্থ')
                    )
                  ))
              )
            )
      ...
      // HOME - Services Grid
      currentScreen === 'home' && React.createElement('div', { className: 'space-y-6 animate-fade-in' },
        ...
        React.createElement('div', { className: 'grid grid-cols-2 md:grid-cols-4 gap-4' },
          Object.keys(parentGroups).map(gid => 
            React.createElement('div', { key: gid, onClick: () => openParentGroup(gid), className: 'service-card' },
              React.createElement('div', { className: 'service-card-icon' }, parentGroups[gid].icon),
        ...
        React.createElement('div', { className: 'grid grid-cols-2 md:grid-cols-5 gap-4' },
          allServices.map(svc => 
            React.createElement('div', { key: svc.id, onClick: () => openDirectService(svc.id), className: 'service-card' },
              React.createElement('div', { className: 'service-card-icon' }, svc.icon),
              React.createElement('h4', { className: 'service-card-title' }, svc.name),
              React.createElement('div', { className: 'text-[10px] mt-1 text-amber-300 font-bold' }, '💰 ' + svc.rate + ' টাকা')
            )
          )
        )
      ),
      
      // SUB SERVICES (প্রায় আগের মতোই - শুধু openDeliveryMessageModal কল করা হয়েছে)
      currentScreen === 'subservices' && selectedParentGroupId && React.createElement('div', { className: 'space-y-6 animate-fade-in' },
        React.createElement('div', { className: 'flex items-center justify-between border-b pb-3' },
          React.createElement('h2', { className: 'text-sm font-bold text-[#0a2e1f]' }, parentGroups[selectedParentGroupId].name),
          React.createElement('button', { onClick: () => { setCurrentScreen('home'); window.history.pushState({}, ''); }, className: 'text-emerald-600 text-xs hover:underline' }, '← পেছনে')
        ),
        React.createElement('div', { className: 'grid grid-cols-3 gap-1.5' }, 
          parentGroups[selectedParentGroupId].children.map(cid => {
            const isSelected = selectedServiceId === cid;
            return React.createElement('div', { 
              key: cid, 
              onClick: () => setSelectedServiceId(cid), 
              className: `p-2 rounded-lg text-center cursor-pointer transition ${isSelected ? 'bg-emerald-600 text-white' : 'bg-slate-800 text-white hover:bg-slate-700'}` 
            },
              React.createElement('div', { className: 'font-semibold text-[11px]' }, serviceForms[cid]?.name),
              React.createElement('div', { className: 'text-[10px] mt-0.5 text-amber-300' }, '💰 ' + serviceForms[cid]?.rate + ' টাকা')
            );
          })
        ),
        selectedServiceId && serviceForms[selectedServiceId] && React.createElement('div', { className: 'bg-white p-3 rounded-xl shadow-md space-y-3 mt-3' },
          React.createElement('div', { className: 'bg-amber-50 border border-amber-200 rounded-md p-1.5 text-center' },
            React.createElement('span', { className: 'text-amber-800 font-bold text-[11px]' }, '💵 খরচ: ', React.createElement('span', { className: 'text-emerald-700 text-sm' }, '৳' + serviceForms[selectedServiceId].rate, ' টাকা'))
          ),
          React.createElement('form', { onSubmit: e => handlePlaceOrder(e, selectedServiceId), className: 'space-y-2' },
            serviceForms[selectedServiceId].fields.map(f => 
              React.createElement('div', { key: f.id, className: 'space-y-0.5' },
                React.createElement('label', { className: 'text-[10px] font-bold text-slate-600' }, f.name),
                React.createElement('input', { type: f.type || 'text', placeholder: f.name, value: formData[f.id] || '', onChange: e => setFormData({ ...formData, [f.id]: e.target.value }), className: 'w-full p-1.5 border rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none text-xs', required: f.required })
              )
            ),
            React.createElement('button', { 
              type: 'submit', 
              disabled: orderSubmitLoading,
              className: 'w-full bg-emerald-600 hover:bg-emerald-700 py-1.5 rounded-lg text-white font-bold transition flex items-center justify-center gap-2 text-xs'
            }, 
              orderSubmitLoading ? React.createElement('div', { className: 'w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin' }) : 
              orderSubmitSuccess ? React.createElement('span', { className: 'check-animation text-xs' }, '✓ অর্ডার সফল!') : 'অর্ডার সাবমিট করুন'
            )
          )
        ),
        // Order History Table
        React.createElement('div', { className: 'bg-white p-1.5 rounded-lg shadow-md mt-3' },
          React.createElement('h3', { className: 'font-bold text-slate-800 mb-1 text-[10px]' }, '📋 আপনার অর্ডার ইতিহাস'),
          React.createElement('div', { className: 'overflow-x-auto' },
            React.createElement('table', { className: 'order-history-table' },
              React.createElement('thead', null,
                React.createElement('tr',
                  React.createElement('th', { className: 'text-[10px] p-1' }, 'ক্রমিক'),
                  React.createElement('th', { className: 'text-[10px] p-1' }, 'সার্ভিস'),
                  React.createElement('th', { className: 'text-[10px] p-1' }, 'তথ্য'),
                  React.createElement('th', { className: 'text-[10px] p-1' }, 'মূল্য'),
                  React.createElement('th', { className: 'text-[10px] p-1' }, 'স্ট্যাটাস'),
                  React.createElement('th', { className: 'text-[10px] p-1' }, 'ডেলিভারি')
                )
              ),
              React.createElement('tbody', null,
                subGroupOrdersList.length === 0 ? 
                  React.createElement('tr', null, React.createElement('td', { colSpan: 6, className: 'p-2 text-center text-slate-400 text-[10px]' }, 'কোনো অর্ডার ইতিহাস নেই')) :
                  subGroupOrdersList.map((o, idx) => {
                    const meta = serviceForms[o.serviceId];
                    return React.createElement('tr', { key: o.orderId },
                      React.createElement('td', { className: 'text-center text-[10px] p-1' }, idx + 1),
                      React.createElement('td', { className: 'font-medium text-slate-700 text-[10px] p-1' }, meta?.name || o.serviceId),
                      React.createElement('td', { className: 'p-1' }, React.createElement('button', { onClick: () => openOrderDetailsModal(o.formData), className: 'order-view-btn text-[9px] py-0.5 px-1.5' }, '👁️ তথ্য')),
                      React.createElement('td', { className: 'font-bold text-emerald-700 text-[10px] p-1' }, '৳' + o.amount),
                      React.createElement('td', { className: 'p-1' }, 
                        o.status === 'pending' ? React.createElement('span', { className: 'order-status-pending text-[9px] py-0.5 px-1.5' }, '⏳ পেন্ডিং') :
                        o.status === 'delivered' ? React.createElement('span', { className: 'order-status-delivered text-[9px] py-0.5 px-1.5' }, '✅ সম্পন্ন') :
                        React.createElement('span', { className: 'order-status-cancelled text-[9px] py-0.5 px-1.5' }, '❌ বাতিল')
                      ),
                      React.createElement('td', { className: 'p-1' },
                        o.status === 'delivered' ? (
                          o.deliveryLink ? React.createElement('a', { href: o.deliveryLink, target: '_blank', className: 'order-download-btn text-[9px] py-0.5 px-1.5' }, '📥 ডাউনলোড') :
                          o.deliveryText ? React.createElement('button', { onClick: () => openDeliveryMessageModal(o.deliveryText), className: 'order-message-btn text-[9px] py-0.5 px-1.5' }, '💬 মেসেজ') :
                          React.createElement('span', { className: 'text-slate-400 text-[9px]' }, 'ডেলিভারড')
                        ) : React.createElement('span', { className: 'text-slate-400 text-[9px]' }, 'অপেক্ষমান')
                      )
                    );
                  })
              )
      ...
      // DIRECT SERVICE (প্রায় আগের মতোই)
      currentScreen === 'direct_service' && selectedServiceId && React.createElement('div', { className: 'space-y-6 animate-fade-in' },
        React.createElement('div', { className: 'flex items-center justify-between border-b pb-3' },
          React.createElement('h2', { className: 'text-sm font-bold text-[#0a2e1f]' }, serviceForms[selectedServiceId]?.name),
          React.createElement('button', { onClick: () => { setCurrentScreen('home'); window.history.pushState({}, ''); }, className: 'text-emerald-600 text-xs hover:underline' }, '← পেছনে')
        ),
        React.createElement('div', { className: 'bg-white p-3 rounded-xl shadow-md max-w-2xl mx-auto' },
          React.createElement('div', { className: 'bg-amber-50 border border-amber-200 rounded-md p-1.5 text-center mb-3' },
            React.createElement('span', { className: 'text-amber-800 font-bold text-[11px]' }, '💵 এই সার্ভিসের জন্য খরচ হবে: ', React.createElement('span', { className: 'text-emerald-700 text-sm' }, '৳' + serviceForms[selectedServiceId].rate, ' টাকা'))
          ),
          React.createElement('form', { onSubmit: e => handlePlaceOrder(e, selectedServiceId), className: 'space-y-3' },
            serviceForms[selectedServiceId].fields.map(f => 
              React.createElement('div', { key: f.id, className: 'space-y-0.5' },
                React.createElement('label', { className: 'text-[10px] font-bold text-slate-600' }, f.name),
                React.createElement('input', { type: f.type || 'text', placeholder: f.name, value: formData[f.id] || '', onChange: e => setFormData({ ...formData, [f.id]: e.target.value }), className: 'w-full p-1.5 border rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none text-xs', required: f.required })
      ...
      currentScreen === 'admin' && React.createElement('div', { className: 'space-y-6' },
        ...
        React.createElement('div', { className: 'flex flex-wrap gap-2 bg-slate-100 p-2 rounded-xl' },
          React.createElement('button', { onClick: () => { setAdminActiveTab('dashboard'); loadAdminStats(); }, className: `px-3 py-1.5 rounded-lg text-xs font-medium transition ${adminActiveTab === 'dashboard' ? 'bg-white shadow text-emerald-700' : 'hover:bg-slate-200'}` }, '📊 ড্যাশবোর্ড'),
          React.createElement('button', { onClick: () => { setAdminActiveTab('orders'); loadAdminOrders(); }, className: `px-3 py-1.5 rounded-lg text-xs font-medium transition ${adminActiveTab === 'orders' ? 'bg-white shadow text-emerald-700' : 'hover:bg-slate-200'}` }, '📦 অর্ডার'),
          React.createElement('button', { onClick: () => { setAdminActiveTab('users'); loadAdminUsers(); }, className: `px-3 py-1.5 rounded-lg text-xs font-medium transition ${adminActiveTab === 'users' ? 'bg-white shadow text-emerald-700' : 'hover:bg-slate-200'}` }, '👥 ইউজার'),
          React.createElement('button', { onClick: () => { setAdminActiveTab('recharges'); loadAdminRecharges(); }, className: `px-3 py-1.5 rounded-lg text-xs font-medium transition ${adminActiveTab === 'recharges' ? 'bg-white shadow text-emerald-700' : 'hover:bg-slate-200'}` }, '💰 রিচার্জ'),
          React.createElement('button', { onClick: () => setAdminActiveTab('notice'), className: `px-3 py-1.5 rounded-lg text-xs font-medium transition ${adminActiveTab === 'notice' ? 'bg-white shadow text-emerald-700' : 'hover:bg-slate-200'}` }, '📢 নোটিশ')
        ),
        
        adminActiveTab === 'dashboard' && React.createElement('div', { className: 'grid grid-cols-2 md:grid-cols-5 gap-3' },
          React.createElement('div', { className: 'bg-white p-3 rounded-xl text-center shadow-sm' }, React.createElement('h3', { className: 'text-slate-400 text-[10px] uppercase' }, 'মোট অর্ডার'), React.createElement('p', { className: 'text-xl font-bold text-slate-800' }, adminStats.total)),
          React.createElement('div', { className: 'bg-white p-3 rounded-xl text-center shadow-sm' }, React.createElement('h3', { className: 'text-amber-500 text-[10px] uppercase' }, 'পেন্ডিং'), React.createElement('p', { className: 'text-xl font-bold text-amber-500' }, adminStats.pending)),
          React.createElement('div', { className: 'bg-white p-3 rounded-xl text-center shadow-sm' }, React.createElement('h3', { className: 'text-emerald-600 text-[10px] uppercase' }, 'সম্পন্ন'), React.createElement('p', { className: 'text-xl font-bold text-emerald-600' }, adminStats.delivered)),
          React.createElement('div', { className: 'bg-white p-3 rounded-xl text-center shadow-sm' }, React.createElement('h3', { className: 'text-red-500 text-[10px] uppercase' }, 'বাতিল'), React.createElement('p', { className: 'text-xl font-bold text-red-500' }, adminStats.cancelled)),
          React.createElement('div', { className: 'bg-emerald-900 p-3 rounded-xl text-center shadow-sm' }, React.createElement('h3', { className: 'text-emerald-400 text-[10px] uppercase' }, 'রেভিনিউ'), React.createElement('p', { className: 'text-xl font-bold text-emerald-300' }, '৳' + adminStats.revenue))
        ),
        
        adminActiveTab === 'recharges' && React.createElement('div', { className: 'bg-white p-3 rounded-xl shadow-md' },
          React.createElement('div', { className: 'flex justify-between items-center mb-2' }, React.createElement('h3', { className: 'font-bold text-xs' }, 'রিচার্জ রিকোয়েস্ট'), React.createElement('button', { onClick: loadAdminRecharges, className: 'text-[10px] text-emerald-600' }, '↻ রিফ্রেশ')),
          React.createElement('div', { className: 'overflow-x-auto' },
            React.createElement('table', { className: 'order-history-table' },
              React.createElement('thead', null, React.createElement('tr', 
                React.createElement('th', { className: 'text-[10px] p-1' }, 'ID'), React.createElement('th', { className: 'text-[10px] p-1' }, 'ইউজার'), React.createElement('th', { className: 'text-[10px] p-1' }, 'পরিমাণ'), React.createElement('th', { className: 'text-[10px] p-1' }, 'Txn ID'), React.createElement('th', { className: 'text-[10px] p-1' }, 'স্ট্যাটাস'), React.createElement('th', { className: 'text-[10px] p-1' }, 'অ্যাকশন')
              )),
              React.createElement('tbody', null, adminRecharges.map(rc => 
                React.createElement('tr', { key: rc.rechargeId },
                  React.createElement('td', { className: 'text-[10px] p-1' }, '#' + rc.rechargeId),
                  React.createElement('td', { className: 'text-[10px] p-1' }, rc.name),
                  React.createElement('td', { className: 'font-bold text-[10px] p-1' }, '৳' + rc.amount),
                  React.createElement('td', { className: 'font-mono text-[9px] p-1' }, rc.transactionId),
                  React.createElement('td', { className: 'p-1' }, 
                    rc.status === 'pending' ? React.createElement('span', { className: 'order-status-pending text-[9px] py-0.5 px-1.5' }, 'পেন্ডিং') :
                    rc.status === 'approved' ? React.createElement('span', { className: 'order-status-delivered text-[9px] py-0.5 px-1.5' }, 'অনুমোদিত') :
                  ...
                  React.createElement('td', { className: 'p-1' }, rc.status === 'pending' && React.createElement(React.Fragment, null,
                    React.createElement('button', { onClick: () => handleUpdateRechargeStatus(rc.rechargeId, 'approved'), className: 'order-download-btn text-[9px] py-0.5 px-1.5 mr-1' }, 'অনুমোদন'),
                    React.createElement('button', { onClick: () => handleUpdateRechargeStatus(rc.rechargeId, 'failed'), className: 'order-status-cancelled text-[9px] py-0.5 px-1.5' }, 'রিজেক্ট')
                  ))
                )
              ))
            )
        ...
        adminActiveTab === 'notice' && React.createElement('div', { className: 'bg-white p-4 rounded-xl shadow-md max-w-lg mx-auto' },
          React.createElement('h3', { className: 'font-bold text-sm mb-2' }, '📢 নোটিশ বোর্ড'),
          React.createElement('textarea', { className: 'w-full p-2 border rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none text-sm', rows: 3, value: adminNoticeInput, onChange: e => setAdminNoticeInput(e.target.value), placeholder: 'নোটিশ লিখুন...' }),
          React.createElement('button', { onClick: handleUpdateNoticeSubmit, className: 'mt-3 w-full bg-emerald-700 hover:bg-emerald-600 py-1.5 rounded-full text-white font-bold text-sm transition' }, 'নোটিশ আপডেট করুন')
        )
      )
    ),
    ...
    deliveryModalOpen && React.createElement('div', { className: 'fixed inset-0 bg-black/50 flex justify-center items-center z-[9999]' },
      React.createElement('div', { className: 'bg-white p-4 rounded-xl w-80 max-w-full mx-4' },
        React.createElement('h3, { className: 'font-bold text-sm mb-3' }, 'ডেলিভারি তথ্য (অর্ডার #' + deliveryOrderId + ')'),
        React.createElement('div', { className: 'flex gap-2 mb-3' },
          React.createElement('button', { onClick: () => { setDeliveryDataType('text'); setDeliveryInputVal(''); }, className: `flex-1 py-1.5 rounded-lg text-xs font-medium transition ${deliveryDataType === 'text' ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-600'}` }, 'টেক্সট মেসেজ'),
          React.createElement('button', { onClick: () => { setDeliveryDataType('link'); setDeliveryInputVal(''); }, className: `flex-1 py-1.5 rounded-lg text-xs font-medium transition ${deliveryDataType === 'link' ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-600'}` }, 'ডাউনলোড লিংক')
        ),
        deliveryDataType === 'text' ?
          React.createElement('textarea', { className: 'w-full p-2 border rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none text-xs', rows: 3, value: deliveryInputVal, onChange: e => setDeliveryInputVal(e.target.value), placeholder: 'ডেলিভারি মেসেজ লিখুন...' }) :
          React.createElement('input', { type: 'url', className: 'w-full p-2 border rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none text-xs', value: deliveryInputVal, onChange: e => setDeliveryInputVal(e.target.value), placeholder: 'https://drive.google.com/...' }),
        React.createElement('div', { className: 'flex gap-2 mt-3' },
          React.createElement('button', { onClick: submitCustomDelivery, className: 'flex-1 bg-emerald-600 hover:bg-emerald-700 py-1.5 rounded-lg text-white font-bold text-xs transition' }, 'জমা দিন'),
          React.createElement('button', { onClick: () => setDeliveryModalOpen(false), className: 'flex-1 bg-slate-200 hover:bg-slate-300 py-1.5 rounded-lg font-medium text-xs transition' }, 'বাতিল')
        )
      )
    )
  );
